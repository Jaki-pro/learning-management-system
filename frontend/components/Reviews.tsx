...
const ReviewSection = () => { 
  const customStyles = `
    ...
    @keyframes scroll-x {
      /* The total width needed is (card width + gap) * number of original items */
      /* Since the animation is applied to the duplicated content, we only move the width of the original set. */
      from { transform: translateX(0); }
      to { transform: translateX(calc(-1 * (300px + 1.5rem) * ${initialReviews.length})); }
    }

    /* Define the Tailwind utility class */
    .animate-scroll-x {
      animation: scroll-x 40s linear infinite; /* 40s duration for smooth scrolling */
    }
  `;

  // --- Theme Setup (Assuming Tailwind Dark/Light mode classes) ---
    ...
    <div className={backgroundClass}>
      ...
      <div className="w-full max-w-7xl">
        <div className="relative w-full overflow-hidden">
          ...
          <div className={`flex animate-scroll-x gap-6 ${cardClasses} hover:[animation-play-state:paused]`}>
            {reviews.map((r, i) => (
              <motion.div
                key={i} 
